package vjyourself4.comp{
	...
	*****************************************************************************/
	public class CompFlipFlow{
		...
		public var mapsName:String = "Test";
		public var upscaling:Number=2;
		...
		*****************************************************************************************/
		public function init(){
			vis = new Sprite();
			for( var i in params) if((i!="n")&&(i!="cn"))this[i]=params[i];
			maps=CompFlipFlowMaps["maps"+mapsName];

			if(ns.sys.io.gamepad.enabled){
				man=ns.sys.io.gamepad.manager;
			...
			dimX=ns.sys.screen.wDimX;
			dimY=ns.sys.screen.wDimY;
			ppi=ns.sys.screen.ppi;
			dimXin=dimX/ppi;
			dimYin=dimY/ppi;
			...
			// Events + Init ****************************************************
			if(ns.sys.io.touch.enabled){
				Multitouch.inputMode = MultitouchInputMode.GESTURE;
				ns.sys.screen.stage.addEventListener(MouseEvent.MOUSE_DOWN, onMouseDown);
				ns.sys.screen.stage.addEventListener(MouseEvent.MOUSE_UP, onMouseUp);
				ns.sys.screen.stage.addEventListener(TransformGestureEvent.GESTURE_ZOOM,onZoom,0,0,1);
				ns.sys.screen.stage.addEventListener(TransformGestureEvent.GESTURE_ROTATE,onRotate,0,0,1);
			}
			...
			visMap=new Sprite();
			vis.addChild(visMirrors);
			createMirrors();

			titleHolder = new Sprite();
			vis.addChild(titleHolder);
		...
		public function pause(v:Boolean){
			if(playing && !v){
				if(ns.sys.io.touch.enabled){
				ns.sys.screen.stage.removeEventListener(MouseEvent.MOUSE_DOWN, onMouseDown);
				ns.sys.screen.stage.removeEventListener(MouseEvent.MOUSE_UP, onMouseUp);
				}
			}
			if(!playing && v){
				if(ns.sys.io.touch.enabled){
		...
		/*******************************************************************************************
		  MIRRORS
		********************************************************************************************/
		public function toggleUpScaling(e=null){
			if(upscaling==2) upscaling=8;
			else upscaling =2;
			createMirrors();
		}

		public function onButtPlus(e=null){
		if(playing){
			switch(flipMode){
				case "Grid":
				if(mirrNum<12){
		...
		public function onButtMinus(e=null){
			if(playing){
			switch(flipMode){
				case "Grid":
				if(mirrNum>2){
		...
		public function onButtFlip(e=null){
		if(playing){
		...
		******************************************************************************************/
		...
		function onMouseDown(e){
			inputMode="touch";
			mouseDown=true;
			touchX0=ns.sys.screen.stage.mouseX;
			touchY0=ns.sys.screen.stage.mouseY;
			trace("M DOwn");
		}
		...
		public function onButtNext(e=null){if(playing)setMap((mapInd+1)%maps.length);}
		public function onButtPrev(e=null){if(playing)setMap((mapInd-1+maps.length)%maps.length);}
		
		
		public function setMap(ind){
			mapInd=ind;
			mapM = maps[mapInd];
		...
		public function toggleLayer0(e=null){if(layers.length>0) layers[0].obj.vis.visible=!layers[0].obj.vis.visible;}
		public function toggleLayer1(e=null){if(layers.length>1) layers[1].obj.vis.visible=!layers[1].obj.vis.visible;}
		public function toggleLayer2(e=null){if(layers.length>2) layers[2].obj.vis.visible=!layers[2].obj.vis.visible;}
		public function toggleLayer3(e=null){if(layers.length>3) layers[3].obj.vis.visible=!layers[3].obj.vis.visible;}
		...
		**************************************************************************************/
		...
		public function onEF(e=null){
			...
			//consoleVal.tfVal.text=inputMode;
			...
			if(playing){
				...
				if(inputMode=="touch"){
					touchSX=ns.sys.screen.stage.mouseX-touchX0;
					touchSY=ns.sys.screen.stage.mouseY-touchY0;
					touchX0=ns.sys.screen.stage.mouseX;
					touchY0=ns.sys.screen.stage.mouseY;
				}
				trace(inputMode+" "+touchSX+" "+touchSY);
		...
		// DRAW MAP ----------------------------------------------------------------------------------
		public function clearBack(e=null){
			drawBmpD.fillRect(drawBmpD.rect,0x000000);
		}
		function drawMap(){
