package vjyourself4.three{
	...
	public class WaveRing{
		...
		{
			cn:"vjyourself4.draw.DrawWave",
			name:"DrawWave",categories:"Draw",
			desc:"",
			//////////////////////////////////////////////
			params:{
				baseCurve:{vals:"circle,line"},
				doFill:{cn:"Boolean"},
				doLine:{cn:"Boolean"},
				doStars:{cn:"Boolean"},
				lineColor1:{},
			...
			system:{musicMeta:{},enterframe:{}},
			needs:{},
			//////////////////////////////////////////////
			vis:[{n:"vis",origo:"center",cn:"flash.display.Sprite"}],
			viscont:[],
			data:{},
			functions:{},
			events:{}
		...
		public function onEF(e=null){if(musicMeta.waveData!=null) draw(musicMeta.waveData);}
		...
		function curveCoord(p):Object{
			...
			switch(baseCurve){
				...
				case "spots":
				if(spots.length>=2){
					var p0=vis.globalToLocal(spots[0].localToGlobal(new Point(0,0)));
					var p1=vis.globalToLocal(spots[1].localToGlobal(new Point(0,0)));
					p.x=p0.x+(p1.x-p0.x)*x-(p1.y-p0.y)*y;
					p.y=p0.y+(p1.y-p0.y)*x+(p1.x-p0.x)*y;
				}
				break;
				case "circle":
